...
- PostgreSQL 17 + pgvector on Supabase (us-west-2). Env `ALICE_PG_URL` is the **only** DB and is **required** — `libDbPool.ts` throws on load if it is unset, so a fresh shell that forgets to source `.env` fails loudly instead of silently hitting some default. The canonical pooler URL lives in `.env`; source it (`set -a; source .env; set +a`) before any `psql` or script run. No local Postgres is supported, expected, or maintained. Schema `alice`, `search_path = alice,public`.
...
- Every subject's daily question is drawn from `tb_question_corpus` by `getOrCreateTodayQuestion` (`src/lib/libQuestionFlow.ts`). One shared corpus, advanced on answer: the function picks the lowest `corpus_question_id` the subject has not yet been served and inserts today's row, on first page load of `/today` or `/subject/today`. No cron, no scheduler, no pre-planting. The "one question per day" rule is enforced by `tb_questions.UNIQUE (subject_id, scheduled_for)`: once today's row exists it persists, once it has a response the same row keeps returning via `existing_response_text` until the subject's local-TZ midnight rolls the date forward. When every active corpus row has been served at least once, the function falls back to the row served longest ago (so corpus exhaustion produces repeats, never empty days). Grow the corpus via `npm run corpus:refresh` + `npm run corpus:approve <file>`. The health endpoint surfaces `activeCorpusEmpty` as the one critical anomaly the operator must act on.
