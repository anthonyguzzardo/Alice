...
- Every subject's daily question is drawn from `tb_question_corpus` by `scheduleQuestionForSubject` (`src/lib/libScheduler.ts`). Round-robin no-repeat; once unseen runs out, the scheduler picks the corpus row with the oldest last-seen date (so corpus exhaustion produces repeats, never empty days). Three planting paths layer on top of one shared scheduler: nightly cron (`src/scripts/schedule-questions.ts`) pre-plants tomorrow for every active subject (owner included); on-demand calls inside `/api/today` and `/api/subject/today` self-heal if the cron is late or missing; manual `npm run corpus:refresh` + `npm run corpus:approve <file>` (additive, never overwrites scheduled rows) when you want to grow the corpus. No per-submission auto-generation. The health endpoint surfaces `subjectsMissingToday`, `subjectsMissingTomorrow`, and `activeCorpusEmpty` as anomalies — the tertiary defense behind the scheduler self-heal and the cron.
