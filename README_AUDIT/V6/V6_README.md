...
- **Trajectory engine** — pure math on raw per-session data (no AI). Collapses behavioral and language-shape metrics into 4 z-scored dimensions (engagement, processing, revision, structure), computes convergence and phase detection. Runs on deterministic, frozen-at-submission data independent of Bob's visual interpretation. See `BOB.md` for details.
