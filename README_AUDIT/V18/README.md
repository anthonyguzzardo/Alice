...
- **Hold time / flight time decomposition** — Kim et al. (2024, JMIR). Hold time (keydown→keyup) measures motor execution; flight time (keyup→next keydown) measures cognitive planning. Flight time alone achieved 97.9% sensitivity, 94.7% specificity for MCI detection, outperforming MoCA-K (Youden Index 0.947 vs. 0.469). The decomposition separates motor and cognitive signals that inter-key interval collapses into a single metric.
...
On days with both a calibration and journal session, the system computes a **same-day session delta** — the behavioral difference between neutral and reflective writing, controlling for every daily confound (sleep, stress, device, time-of-day). This implements Pennebaker's (1986) expressive writing paradigm: the only variable that changed was the prompt. The delta tracks 10 research-backed dimensions: first-person density (Newman 2003), cognitive density (Vrij), hedging density, typing speed, commitment ratio, large deletions (Faigley & Witte), keystroke interval (Epp et al.), P-burst length (Chenoweth & Hayes), hold time (Kim et al. 2024 — motor execution shift), and flight time (Kim et al. 2024 — cognitive planning shift). The hold time and flight time deltas are the core motor/cognitive decomposition: if a reflective question increases flight time (more cognitive planning between keys) without changing hold time (motor execution stays stable), that's pure cognitive load from the prompt. After 15+ days of history, the system contextualizes whether today's delta is within or outside the person's typical range — a widening delta means the question provoked more behavioral shift than usual.
...
- **Hold time / flight time decomposition** (`hold_time_mean`, `hold_time_std`, `flight_time_mean`, `flight_time_std`) — Kim et al. (2024, JMIR). IKI decomposed into motor execution (hold time: keydown→keyup) and cognitive planning (flight time: keyup→next keydown). Auto-repeat events filtered via `e.repeat` to prevent OS key-repeat from contaminating the signal. Hold time capped at 2s, flight time at 5s.
