...
**Anthony Guzzardo**
April 2026 (v3, updated April 23)

*v3 changelog: Contamination boundary now formally implemented (Contamination Boundary v1, cryptographic per-session attestation). Embedding pipeline migrated from vendor API to self-hosted Qwen3-Embedding-0.6B (FP32, bit-reproducible, weights archived by SHA-256). Semantic signal computation migrated to the same Rust engine as the dynamical, motor, and process families; no third-party LLM is invoked anywhere in the signal pipeline. The only LLM call in the system is operator-run, off-band, user-agnostic question generation populating a shared question corpus, and it never receives or analyzes subject response data. Semantic baseline infrastructure operational (Welford running distributions, topic-matched z-scoring, minimum-n gating). Session count updated (10 non-calibration, 55 calibration). Semantic dimension count corrected (16 state dimensions + 7 baseline-tracked signals). Methods provenance citations added throughout. Calibration delta language softened from construct claim to design description. Cross-paper canonical-source citations added (Guzzardo 2026b for construct replacement, Guzzardo 2026g for irreversibility).*

---

*Author's note: The author has built and operates a longitudinal journaling system (Alice) that implements the design constraints discussed in Sections 7 and 8, with cryptographically attested contamination boundaries (contamination-boundary-v1.md), archived embedding methodology (embedding-methods.md), and bit-reproducible signal computation (METHODS_PROVENANCE.md). As of v3, Alice captures keystroke dynamics at microsecond precision, computes nonlinear dynamical signals (permutation entropy, DFA, recurrence quantification analysis, transfer entropy), motor biometric signals (ex-Gaussian decomposition, sample entropy, adjacent hold-time covariance), and linguistic content analysis across 16 semantic dimensions in the per-session state table and 7 additional baseline-tracked longitudinal signals (idea density, lexical sophistication, epistemic stance, integrative complexity, deep cohesion, text compression ratio, referential cohesion) via a single compiled Rust engine — no third-party LLM is invoked at any point in the signal pipeline, and maintains longitudinal self-referential baselines with same-day calibration deltas. Readers should apply additional scrutiny to those sections, where the instrument-gap analysis overlaps with design decisions already implemented in that system. The arguments in Sections 2 through 6  -  the automaticity threshold, the demographic confound, the self-resolving timeline, the clock-drawing precedent, and the paradigm shift to self-referential baselines  -  do not depend on any specific implementation and should be evaluated on their own terms.*
...
The metrics for this paradigm do not yet exist at validated scale. This is not a gap in the literature. It is a consequence of the data never having been available. You cannot develop intra-individual longitudinal baselines from a 45-minute lab session with 23 participants who hunt and peck on a touchscreen. You need dense, daily, fluent data from the same person over years. The infrastructure for computing such baselines is now buildable (see Guzzardo 2026a, Section 8 v3 note), but the analysis layers that would consume them (drift detection, trait-vs-state signal classification, baseline model sophistication beyond stationary assumptions) are formally deferred pending data depth (METHODS_PROVENANCE.md DEF-001 through DEF-003). The population that can produce that data at the age where cognitive decline becomes clinically relevant does not yet exist  -  and the closing window described in Section 4 means that the population that will exist may not produce it unassisted either.
...
The signals that hold up for longitudinal self-referential tracking share a common property: they measure process rather than product, and they generate stable within-person baselines against which deviation can be detected. *v3 note: All signals described in this section are implemented and computing in Alice's signal pipeline with bit-reproducible output verified by CI-enforced cross-build snapshot testing (see METHODS_PROVENANCE.md INC-001, INC-002, INC-004 for the signal computation audit trail). Keystroke dynamics and motor-cognitive decomposition are computed via a compiled Rust engine (napi-rs) at sub-2ms latency on 500-keystroke streams. Linguistic signals are computed server-side. Calibration deltas are computed on days with both calibration and journal sessions. Stability analysis of signal behavior across sessions requires accumulated data depth not yet available.*
...
**Calibration deltas** provide a within-session control. When a person completes both a neutral calibration prompt and a substantive journal entry on the same day, the difference between their behavioral signals on the two tasks provides a same-day within-person contrast that controls for transient state variation (fatigue, illness, mood)  -  analogous to the Pennebaker expressive writing paradigm (1997). The construct claim that this contrast isolates cognitive effort specifically requires data depth not yet available.
...
At the time of v1 publication (April 17, 2026), no existing tool combined keystroke dynamics, linguistic content analysis, and longitudinal self-referential tracking in a single instrument. As of v2, a working prototype (Alice) satisfies each of the design constraints enumerated below, though it remains at n=1 with 10 non-calibration sessions and 55 calibration sessions fully instrumented. The instrument gap as described here has been narrowed in implementation but remains open as a validated research paradigm.
...
*v3 note: Alice satisfies or addresses all seven constraints enumerated above. Process capture records every keystroke event at microsecond precision via `performance.now()`, storing complete keystroke streams and event logs for post-hoc analysis. Content analysis computes 16 semantic dimensions in the per-session state table and 7 additional baseline-tracked longitudinal signals (idea density, lexical sophistication, epistemic stance, integrative complexity, deep cohesion, text compression ratio, referential cohesion) alongside 51 dynamical signals (organized into 11 theoretical families: scaling/fractality, spectral, ordinal, transition networks, recurrence, causal emergence, PID, criticality, dynamic modes, pause mixture, and transfer entropy), 17 motor signals, and 9 process signals per session. The signal architecture provides independent cross-validation pairs per measurement dimension (DFA + MF-DFA + spectral slope for scaling; PE + CECP + LZ for complexity; RQA + recurrence networks for phase-space topology), addressing the methodological gap documented in METHODS_PROVENANCE.md INC-012. Longitudinal architecture stores all raw data in PostgreSQL with pgvector-indexed embeddings via a self-hosted open-weights model (Qwen3-Embedding-0.6B, weights archived by SHA-256, FP32 bit-reproducible inference; see embedding-methods.md) for semantic similarity search across the full journal history. Semantic baselines use Welford's online algorithm for running per-signal mean and variance, with topic-matched z-scoring via HNSW retrieval and minimum-n gating at 10 sessions (drift detection, tier separation of stable vs volatile signals, and baseline model sophistication are deferred pending data depth; see METHODS_PROVENANCE.md DEF-001 through DEF-003). Calibration sessions use neutral prompts to establish same-day behavioral baselines, providing a within-person contrast that controls for transient state variation. AI-mediation prevention is implemented via Contamination Boundary v1, with cryptographic per-session attestation of audited code paths and git commit hash at session-write time (see contamination-boundary-v1.md). Modality awareness is held as a design principle whose validation depends on future cross-modality migration work. The system is single-user, local-first, and has been accumulating data since April 2026. It remains n=1 and unvalidated. The gap between "implemented" and "validated" is the work that remains. Adversarial reconstruction validation (Option F) confirms that the 40-dimension behavioral comparison produces a per-family residual map: multifractal structure (MF-DFA spectrum width) is the most ghost-resistant dimension (residual 4x larger than any other family), while ordinal statistics are nearly perfectly reproducible, establishing the boundary between what the instrument measures about the mind versus what it measures about the distribution. Methods provenance for the signal computation pipeline is documented in METHODS_PROVENANCE.md (INC-001 through INC-012).*
