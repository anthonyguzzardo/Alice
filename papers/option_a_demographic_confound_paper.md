...
*v3 changelog: Contamination boundary now formally implemented (Contamination Boundary v1, cryptographic per-session attestation). Embedding pipeline migrated from vendor API to self-hosted Qwen3-Embedding-0.6B (FP32, bit-reproducible, weights archived by SHA-256). Semantic baseline infrastructure operational (Welford running distributions, topic-matched z-scoring, minimum-n gating). Session count updated (10 non-calibration, 55 calibration). Semantic dimension count corrected (16 state dimensions + 7 baseline-tracked signals). Methods provenance citations added throughout. Calibration delta language softened from construct claim to design description. Cross-paper canonical-source citations added (Guzzardo 2026b for construct replacement, Guzzardo 2026g for irreversibility).*

---

*Author's note: The author has built and operates a longitudinal journaling system (Alice) that implements the design constraints discussed in Sections 7 and 8, with cryptographically attested contamination boundaries (contamination-boundary-v1.md), archived embedding methodology (embedding-methods.md), and bit-reproducible signal computation (METHODS_PROVENANCE.md). As of v3, Alice captures keystroke dynamics at microsecond precision, computes nonlinear dynamical signals (permutation entropy, DFA, recurrence quantification analysis, transfer entropy) and motor biometric signals (ex-Gaussian decomposition, sample entropy, adjacent hold-time covariance) via a compiled Rust engine, performs linguistic content analysis across 16 semantic dimensions in the per-session state table and 7 additional baseline-tracked longitudinal signals (idea density, lexical sophistication, epistemic stance, integrative complexity, deep cohesion, text compression ratio, referential cohesion), and maintains longitudinal self-referential baselines with same-day calibration deltas. Readers should apply additional scrutiny to those sections, where the instrument-gap analysis overlaps with design decisions already implemented in that system. The arguments in Sections 2 through 6  -  the automaticity threshold, the demographic confound, the self-resolving timeline, the clock-drawing precedent, and the paradigm shift to self-referential baselines  -  do not depend on any specific implementation and should be evaluated on their own terms.*
