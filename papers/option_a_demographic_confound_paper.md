...
*v3 note: Alice satisfies or addresses all seven constraints enumerated above. Process capture records every keystroke event at microsecond precision via `performance.now()`, storing complete keystroke streams and event logs for post-hoc analysis. Content analysis computes 16 semantic dimensions in the per-session state table and 7 additional baseline-tracked longitudinal signals (idea density, lexical sophistication, epistemic stance, integrative complexity, deep cohesion, text compression ratio, referential cohesion) alongside 13 dynamical signals, 12 motor signals, and 9 process signals per session. Longitudinal architecture stores all raw data in PostgreSQL with pgvector-indexed embeddings via a self-hosted open-weights model (Qwen3-Embedding-0.6B, weights archived by SHA-256, FP32 bit-reproducible inference; see embedding-methods.md) for semantic similarity search across the full journal history. Semantic baselines use Welford's online algorithm for running per-signal mean and variance, with topic-matched z-scoring via HNSW retrieval and minimum-n gating at 10 sessions (drift detection, tier separation of stable vs volatile signals, and baseline model sophistication are deferred pending data depth; see METHODS_PROVENANCE.md DEF-001 through DEF-003). Calibration sessions use neutral prompts to establish same-day behavioral baselines, providing a within-person contrast that controls for transient state variation. AI-mediation prevention is implemented via Contamination Boundary v1, with cryptographic per-session attestation of audited code paths and git commit hash at session-write time (see contamination-boundary-v1.md). Modality awareness is held as a design principle whose validation depends on future cross-modality migration work. The system is single-user, local-first, and has been accumulating data since April 2026. It remains n=1 and unvalidated. The gap between "implemented" and "validated" is the work that remains. Methods provenance for the signal computation pipeline is documented in METHODS_PROVENANCE.md (INC-001 through INC-010).*
