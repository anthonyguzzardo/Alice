...
None of these signals are available from finished text. Each measures a different dimension of the cognitive work of production, and each generates a within-person baseline against which deviation can be detected.
...
The resistance of motor-level features becomes more precise when stated in terms of what they measure. Detrended fluctuation analysis of keystroke timing extracts the fractal scaling exponent of the production process — a measure of long-range temporal correlation that reflects whether cognitive engagement is structured (exponent near 0.8, characteristic "pink noise") or flattened toward independence (exponent near 0.5, "white noise"). This exponent is invariant to what words are produced and to what linguistic patterns the writer has internalized from AI exposure outside the controlled environment. It measures the temporal organization of the act of writing, not the quality or style of the product. A person whose daily emails are drafted by an LLM and whose prose style has converged toward model output will still produce their own fractal scaling signature when typing unassisted, because the signature arises from the coupling between cognition and motor execution — a coupling that no language model participates in.
