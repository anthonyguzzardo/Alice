...
*Author's note: The author has built and operates a longitudinal journaling system (Alice) that implements the design constraints discussed in Section 8 of this paper, including cryptographic contamination boundary attestation (contamination-boundary-v1.md), self-hosted archivable embeddings for topic-matched longitudinal comparison (embedding-methods.md), and bit-reproducible signal computation (METHODS_PROVENANCE.md). Readers should apply additional scrutiny to sections where the instrument-gap analysis overlaps with design decisions implemented in that system. The arguments about cognitive reserve, the retirement literature, and the AI-offloading problem do not depend on any specific implementation and should be evaluated on their own terms.*
...
This question has never been empirically investigated, for a specific reason: the data has only just begun to exist. Pennebaker's paradigm has been studied in hundreds of papers over four decades. Every one of them measures outcomes — symptom reduction, immune markers, re-employment rates, mood scales administered before and after. No study has captured the process-level behavioral data generated during the act of therapeutic writing itself. The instrumentation to capture it is now operational (Guzzardo 2026f describes the reconstruction validity framework; METHODS_PROVENANCE.md INC-006 documents the residual as a reproducible scientific artifact), but the longitudinal corpus required to address the question is in its earliest accumulation phase. No one knows what the keystroke signature of therapeutic benefit looks like — whether burst length increases as emotional processing deepens, whether revision depth peaks and then declines as integration occurs, whether the temporal structure of pauses shifts as a person moves from avoidance to confrontation. The expressive writing literature has a forty-year gap between its outcome data and its process data. An instrument that captures keystroke dynamics, pause distributions, revision topology, and production rhythm during daily reflective writing closes that gap by construction, without requiring any clinical claims beyond "here is what the process looks like."

Recurrence quantification analysis offers a candidate operationalization. Laminarity — the tendency of a system to enter a state and remain trapped there — may operationalize the distinction Pennebaker draws between writing that integrates and writing that does not. High laminarity in a keystroke timing series means the writer is producing text from a single cognitive state: the same hesitation pattern, the same production rhythm, repeated without traversal. Low laminarity means movement between states — bursts of fluency giving way to deliberate revision giving way to exploratory pauses. If Pennebaker's mechanism requires the writer to connect feeling to narrative, the process-level signature of that connection is traversal across cognitive states during the session. Laminarity does not measure what the writer says. It measures whether the act of writing is taking them anywhere.
...
**It must capture process, not just product.** The finished text of an AI-assisted writer and an unassisted writer may be indistinguishable. The keystroke dynamics, pause distributions, revision patterns, and production rhythm are not. Mehta and colleagues (2025) demonstrated that keystroke-based classifiers detect AI-assisted writing at 97 to 98 percent F1 score, while content-based detection methods achieve only 7 to 10 percent and human evaluators perform at chance. The signal is in the temporal microstructure of production — hold time, flight time, inter-key interval, P-burst length and distribution, the Faigley-Witte taxonomy of surface versus meaning-level revision.

The depth of process capture matters. Beyond summary statistics, the full keystroke stream supports decompositions that separate motor from cognitive contributions with increasing precision. Ex-Gaussian fitting of flight time distributions (Zulueta et al. 2018, BiAffect) decomposes each session's timing into a Gaussian component (mu and sigma, reflecting motor baseline and motor noise) and an exponential tail (tau, reflecting cognitive slowing). The ratio of tau to mean flight time isolates the cognitive fraction of keystroke timing — a signal that persists regardless of the writer's motor skill level. Adjacent hold-time covariance (Giancardo et al. 2016, neuroQWERTY) captures fine motor coordination between consecutive keystrokes, providing a motor biomarker that is independent of typing speed. Error correction can be decomposed into three phases: detection (the pause before the first delete), execution (the speed of the deletion chain itself), and recovery (the latency from last delete to next inserted character) (Springer 2021). The recovery phase is pure cognitive re-planning — a signal that measures the cost of error, not the error itself. Cursor trajectory during typing pauses — total movement, fidget ratio, stillness proportion — captures involuntary motor behavior during cognitive load (BioCatch cognitive biometrics). Revision distance (Lindgren and Sullivan 2006, ScriptLog) measures how far back from the leading edge a writer reaches to revise, distinguishing deep structural restructuring from surface corrections. Punctuation flight time relative to letter flight time (Plank 2016) isolates syntactic planning load at clause boundaries.

None of these signals are available from finished text. Each measures a different dimension of the cognitive work of production, and each generates a within-person baseline against which deviation can be detected. A production implementation computes these signals via a bit-reproducible deterministic Rust engine with CI-enforced cross-build verification (METHODS_PROVENANCE.md INC-001 through INC-006), including numerically stable ex-Gaussian estimation via MLE/EM (INC-004).

But summary statistics of even these richer signals — mean inter-key interval, standard deviation of pause duration — are lossy in a specific way: they discard the temporal order of the series and treat it as a bag of independent samples. Two sessions with identical average typing speed and identical variability can have completely different sequential structure — one metronomic and habitual, the other exploratory and generative. Nonlinear dynamics methods developed for physiological time series — permutation entropy (Bandt and Pompe 2002), detrended fluctuation analysis (Peng et al. 1994), recurrence quantification (Webber and Zbilut 2005), transfer entropy (Schreiber 2000) — preserve that sequential structure and extract cognitive signatures from it. These methods are well-validated in cardiology, neuroscience, and motor control; their application to keystroke timing series is newer but grounded in the same mathematics. The case for adopting them is not that they are novel but that the summary statistics they would replace are increasingly insufficient: as content-level biomarkers lose validity to AI contamination, the remaining process-level signals must carry more diagnostic weight, and sequence-preserving analysis extracts more information from the same data stream.

But a single estimator per dimension is not enough. If the instrument's claim rests on detecting whether cognitive work is present in the timing data, it needs independent cross-validation: multiple estimators measuring the same construct through different mathematical lenses, so that a genuine shift is distinguishable from estimator-specific artifact. For temporal scaling, DFA provides a monofractal exponent, but multifractal DFA (MF-DFA; Kantelhardt et al. 2002) tests whether the monofractal assumption holds by computing the full singularity spectrum. For complexity, permutation entropy captures rank-order patterns, but the Complexity-Entropy Causality Plane (CECP; Rosso et al. 2007) answers a question PE cannot: whether the complexity is deterministic or stochastic. For recurrence structure, RQA line statistics measure sequential regularity, but graph-theoretic analysis of the same recurrence matrix (Donner et al. 2010) captures topological properties invisible to the line statistics. For the frequency domain, Lomb-Scargle periodogram analysis (Lomb 1976, Scargle 1982) provides an independent check on DFA's scaling exponent and exposes respiratory-frequency entrainment that time-domain methods cannot detect.

The cross-validation architecture also reveals where the instrument is measuring something the person's statistical profile cannot reproduce. A production implementation's adversarial reconstruction system (five Markov/PPM variants of increasing statistical sophistication) shows that multifractal structure is the single most ghost-resistant dimension: the mean residual for MF-DFA spectrum width is 4x larger than any other signal family. This is theoretically coherent. The ghost generates from a single stochastic process, which produces a narrow singularity spectrum. Real typing has broad multifractal structure from the interaction of multiple cognitive processes operating at different time scales. No amount of statistical sophistication in the ghost closes this gap. Conversely, ordinal pattern statistics are nearly perfectly reproducible by the ghost (residual approaching zero), confirming that those patterns reflect the distributional shape rather than the cognitive process.

This dimensional breakdown matters for the reserve-decline framing because reserve erosion from AI offloading would not manifest as a simple shift in one dimension. It would manifest as a pattern across multiple correlated dimensions: narrowing multifractal spectrum (fewer cognitive processes engaged), declining multiscale entropy (loss of complexity across coarse-graining scales), reduced causal emergence (macro-level dynamics becoming less informative than micro), flattening spectral slope (loss of the 1/f structure that characterizes healthy complex systems). With independent cross-validators per dimension, the instrument can detect and decompose this pattern. With single estimators, it cannot.

A final methodological constraint: not all signals are equally trustworthy for longitudinal trend inference. Mangalam and colleagues (2022, Journal of the Royal Society Interface) showed that inter-keystroke interval means are non-ergodic: their time averages diverge from ensemble averages under multiplicative processes. This means IKI-derived means (typing speed, hold time mean, flight time mean) are valid per-session measurements but unreliable as longitudinal trend estimators. Ratios, densities, fractal exponents, and information-theoretic quantities are not affected by this specific non-ergodicity. An instrument designed for decade-scale trajectory detection must classify its signals accordingly and restrict trend inference to the ergodicity-safe subset.

**It must be longitudinal by architecture, not by afterthought.** A single writing session is a snapshot. A thousand writing sessions from the same person over years is a trajectory. The retirement literature's central finding — that disengagement accelerates decline — is invisible in cross-sectional data. It required years of longitudinal tracking to surface. If AI offloading produces an analogous trajectory shift in cognitive reserve accumulation, it will be invisible in any study shorter than the timescale of the effect. The instrument must be designed to accumulate, to build personal baselines, and to detect drift against those baselines over months and years. Topic-matched z-scoring via semantic similarity retrieval (embedding-methods.md documents the archivable embedding specification) allows comparison against thematically similar prior sessions rather than the undifferentiated global baseline.

**It must control the writing environment.** The construct being measured — unmediated cognitive engagement — requires a context in which the writing is actually unmediated. This means no autocomplete, no predictive text, no AI assistance, no paste functionality that would allow importing externally generated text. The controlled environment is not an inconvenience. It is the instrument. Without it, the process data reflects a hybrid human-AI system rather than the person's cognitive state. The contamination boundary must be formally specified and attested per session, not assumed (contamination-boundary-v1.md documents one such specification).
...
One such instrument has been built and is accumulating data. It captures keystroke dynamics during daily unmediated writing, computes process-level signals via bit-reproducible deterministic computation, maintains self-referencing longitudinal baselines with topic-matched z-scoring, and cryptographically attests that each session was captured under verified unassisted conditions (Guzzardo 2026a, Sections 7-8). What it does not yet have is the years of accumulated data that longitudinal validation requires. The architectural feasibility of the instrument is demonstrated. The empirical question — whether the baselines it accumulates will detect the trajectory shifts this paper hypothesizes — can only be answered by time.
...
The resistance of motor-level features becomes more precise when stated in terms of what they measure. Detrended fluctuation analysis of keystroke timing extracts the fractal scaling exponent of the production process — a measure of long-range temporal correlation that reflects whether cognitive engagement is structured (exponent near 0.8, characteristic "pink noise") or flattened toward independence (exponent near 0.5, "white noise"). This exponent is mathematically independent of what words are produced: it operates on inter-keystroke intervals, not text content. A person whose vocabulary has converged toward AI-typical patterns will still produce their own fractal scaling signature when typing unassisted, because the signature arises from the coupling between cognition and motor execution — a coupling that no language model participates in. Whether sustained AI usage could alter the person's typing *habits* in ways that affect IKI patterns even during unmediated sessions is a separate and empirically untested question. The lexical invariance is grounded in the mathematics. The behavioral invariance — whether a person who has spent years accepting suggestions still approaches unassisted composition the same way — remains open.

Some process-level signals are not merely resistant to AI contamination but structurally immune to it, because they measure behaviors in which no language model participates. Cursor movement during typing pauses — fidgeting, drift toward the submit button, stillness — is involuntary motor behavior reflecting cognitive load; it has no analog in AI-assisted writing because the AI does not move the cursor. Postcorrection latency — the time between the last deletion and the next inserted character — measures the cognitive cost of recovering from an error; if AI prevents the error from occurring (via autocomplete or suggestion), the signal vanishes not because it was contaminated but because the cognitive event it measures was eliminated. The disappearance of such signals in a longitudinal record would itself be diagnostic: a person whose postcorrection latency entries go from populated to consistently null is a person who has stopped making and correcting their own errors.
...
Process-level behavioral data captured during verified unmediated writing survives this double contamination. But only in a controlled environment — a context where the writing is known to be unassisted, where autocomplete and paste are disabled, where the person is producing language character by character from their own cognition. The verification must be architectural, not self-reported: a contamination boundary specification that enumerates the audited code paths between keystroke and storage, attested cryptographically per session (contamination-boundary-v1.md). Passive ambient monitoring from daily device use cannot provide this guarantee. The future of cognitive biomarkers in an AI-mediated world may not be passive sensing at all. It may be something closer to a daily controlled cognitive exercise with process-level instrumentation — brief, voluntary, ecologically motivated, and verified as unmediated.
...
Plank, B. (2016). Keystroke dynamics as signal for shallow syntactic parsing. *Proceedings of COLING 2016*, 609-619.
...
Webber, C. L., Jr., & Zbilut, J. P. (2005). Recurrence quantification analysis of nonlinear dynamical systems. In M. A. Riley & G. C. Van Orden (Eds.), *Tutorials in Contemporary Nonlinear Methods for the Behavioral Sciences* (pp. 26-94).
