...
**Dynamical signals.** Permutation entropy at orders 3 through 7 (the PE spectrum, following Bandt and Pompe 2002), providing a multi-scale complexity profile. Detrended fluctuation analysis (DFA) scaling exponent (Peng et al. 1994), characterizing long-range temporal correlations. Multifractal DFA (Kantelhardt et al. 2002), yielding singularity spectrum width, asymmetry, and peak scaling exponent. Temporal irreversibility via KL divergence of forward and backward transition matrices. Lomb-Scargle periodogram (Lomb 1976, Scargle 1982) yielding spectral slope, respiratory-frequency peak, LF/HF ratio, and peak typing frequency. Ordinal pattern extensions: statistical complexity via the Complexity-Entropy Causality Plane (CECP; Rosso et al. 2007), forbidden pattern fraction, amplitude-weighted PE, and Lempel-Ziv complexity. Ordinal pattern transition networks (OPTN; McCullough et al. 2015), yielding transition entropy and forbidden transition count. Recurrence quantification analysis (RQA) metrics including recurrence rate, determinism, laminarity, trapping time, and entropy of diagonal line lengths (Marwan et al. 2007), extended with recurrence time entropy, mean recurrence time, and graph-theoretic recurrence network analysis (Donner et al. 2010) yielding transitivity, average path length, clustering coefficient, and assortativity. Kraskov-Stogbauer-Grassberger (KSG) transfer entropy between IKI series and hold times (Kraskov et al. 2004). Causal emergence via effective information and causal emergence index (Hoel et al. 2013). Partial information decomposition (PID; Williams and Beer 2010) yielding synergy and redundancy between hold and flight channels. Branching ratio and avalanche size exponent as criticality indicators. Dynamic mode decomposition (DMD; Schmid 2010) yielding dominant frequency, decay rate, mode count, and spectral entropy. Pause mixture decomposition via Gaussian mixture model on log-pause durations, yielding component count, motor proportion, and cognitive load index. In total, 51 dynamical signals computed per session.

**Motor signals.** Ex-Gaussian decomposition of the inter-keystroke interval (IKI) distribution via maximum likelihood estimation using expectation-maximization (Lacouture and Cousineau 2008), yielding mu (Gaussian mean), sigma (Gaussian standard deviation), and tau (exponential tail, reflecting attentional lapses). Fisher information trace of the ex-Gaussian fit, quantifying measurement informativeness per session. Per-digraph latency profiles. Hold time and flight time distributions. Sample entropy of the IKI series. Multiscale entropy (MSE; Costa et al. 2005) at scales 1 through 5, yielding a complexity index that captures whether entropy is maintained or degrades under coarse-graining. IKI autocorrelation structure. In total, 17 motor signals computed per session.
...
The motor signals the instrument extracts (sample entropy, multiscale entropy, motor jerk, tempo drift, IKI autocorrelation, Fisher information) are sensitive to the *sequence* of intervals, not just their distribution. Genuine composition produces motor sequences structured by cognitive events. Statistical sampling, even with preserved serial dependence and coupling, produces motor sequences that the instrument reliably distinguishes from real ones. The motor channel detects the mind. Five independent strategies have now failed to close that gap.

The signal expansion adds a second, independent confirmation. Multifractal DFA spectrum width carries a residual of 4.75, the largest of any single signal by a factor of four. The monofractal DFA exponent is reproducible (the ghost can match the average scaling). But the *width* of the singularity spectrum, which measures the diversity of scaling behaviors present in the series, is not. Real typing engages multiple cognitive processes at different time scales, each contributing its own scaling regime. The ghost has one process. This is not a motor signal, nor a dynamical one in the original sense. It is a structural property of the time series that emerges only from multi-scale analysis. Its ghost resistance is evidence that the instrument measures something deeper than any single statistical property: the simultaneous engagement of multiple cognitive systems during composition.
