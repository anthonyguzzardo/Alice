...
The session summary populates ~50 deterministic signals: first-keystroke latency, P-burst metrics, commitment ratio, deletion decomposition (Faigley & Witte), revision timing, active typing speed, lexical diversity (MATTR), inter-key interval mean and std (Epp et al. 2011), hold time and flight time (Kim et al. 2024), keystroke entropy (Ajilore et al. 2025), revision chain topology (Leijten & Van Waes 2013), scroll-back and question-reread counts (Czerwinski et al. 2004), context metadata (device, hour, day-of-week), pause topology, tab-away behavior. All metrics are normalized as personal percentiles — compared against the user's own history, not population norms.
...
- **Per-keystroke event log** — client-side text-snapshot capture (capped at 5000 events per session), persisted to `tb_session_events` for replay.
