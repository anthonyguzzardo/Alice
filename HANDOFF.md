...
11. **JSON.stringify-into-JSONB anti-pattern is widespread.** Discovered while building `libDelete.ts`. Found in `src/lib/`: `libDb.ts:1517-1525` (tb_prompt_traces.recent_entry_ids + 5 sibling JSONB columns), `libReconstruction.ts:299-301` (tb_reconstruction_residuals PE-spectrum columns), `libProfile.ts:310-312` (tb_personal_profile digraph + trigram), `libSignalPipeline.ts:88, 147, 150, 281` (tb_dynamical_signals.pe_spectrum, tb_motor_signals.mse_series + iki_autocorrelation, tb_session_integrity.z_scores). All write `JSON.stringify(obj)` into a JSONB column, which postgres.js then stores as a JSONB *string* (the JSON-encoded form of the object), not a JSONB *object*. Verified empirically: `jsonb_typeof(recent_entry_ids)` returns `'string'` for all 4 existing rows in `tb_prompt_traces`. The read-side compensates: `libDb.ts:1983, 2043-2044` re-stringify if `typeof row.col === 'object'` so downstream signal-pipeline code consistently sees strings. This is a *consistency workaround*, not a fix — the storage shape is inconsistent (some columns may have been written as objects in earlier code), and the workaround papers over it. New code (consent + audit) uses `sql.json(...)` to write proper JSONB objects (post-step-7 fix in libConsent.ts). Migration 040 followup: backfill stringified JSONB columns by `UPDATE ... SET col = col #>> '{}'` (extracts JSON from the string-typed JSONB), then change writers to `sql.json(...)`, then remove the read-side `typeof === 'object'` workaround. Until 040 ships, do NOT mix patterns within a single column — pick one shape per column and stick with it.
