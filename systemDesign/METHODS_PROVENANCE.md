...
### Historical audit + repair (2026-04-28 evening)

Audit ran against production Supabase (us-west-2) immediately after the code fix landed. Of 85 total `tb_responses` rows: 51 had a paired `tb_session_events` row (auditable via inter-table delta), 31 were owner calibrations from April 14-18 with no SE row but immune to the bug by code path (calibration writes never went through `nowStr()`), and 3 were owner journal rows from April 16-18 indeterminate (`saveSessionEvents` was also using `nowStr()` back then so both sides skewed identically and the inter-table check was blind during that window — `saveSessionEvents` switched to the `CURRENT_TIMESTAMP` DEFAULT around April 18-19, opening the audit window).

Within the auditable 51, **9 rows** showed +18000s skew (= exactly the CDT offset, UTC-5), all owner journal rows submitted between April 19 and April 28: response_id 47, 53, 60, 64, 66, 68, 72, 79, 84. Every other auditable row was within sub-second of its session_events partner. Mechanism cross-validated three ways: (1) inter-table delta `r.dttm = trunc_to_second(se.dttm + 5h)` exact for all 9; (2) for response 84 the matching `tb_dynamical_signals` row was timestamped 5h *before* the response — physically impossible since signals are computed *from* the response, confirming the response timestamp was the wrong side; (3) sub-second-precision boundary in `tb_session_events` cleanly cuts on April 18→19 where SE switched from `nowStr()` to the column DEFAULT.

The bug's mechanism turned out to be deterministic per (client environment, code path), not random per request as the original write-up framed it. Owner journals submitted from the developer laptop hit upstream Supabase backends where the connection's session `TimeZone` resolved to a non-UTC value (manifested as exactly `-05:00` = CDT/America-Chicago); ash journals submitted via the production Hetzner box (system TZ = UTC) and all calibrations (different write path) were never affected. The audit window happened to coincide with a stretch where every owner journal write hit a non-UTC backend session, hence 9-of-9 within April 19-28 skewed rather than a probabilistic mix.

Repair committed atomically in a single transaction: `UPDATE tb_responses SET dttm_created_utc = se.dttm_created_utc, dttm_modified_utc = now(), modified_by = 'inc-021-repair' FROM tb_session_events se WHERE se.question_id = r.question_id AND r.response_id IN (47, 53, 60, 64, 66, 68, 72, 79, 84)` returned 9 affected rows. Post-repair audit (same delta check) returned `still_skewed_30s = 0` across the auditable 51. The 3 indeterminate rows from April 16-18 are not repaired — without a second-source timestamp we cannot prove they were skewed, and their `tb_session_events` partners are equally affected if so. They remain documented as "indeterminate, plausibly skewed by the same mechanism, not separately recoverable."

Persisted derived signals (`tb_dynamical_signals`, `tb_motor_signals`, etc.) were not recomputed because they do not depend on `tb_responses.dttm_created_utc` — they take their data from the keystroke stream and store their own `CURRENT_TIMESTAMP`-defaulted creation timestamp. Any future analytical consumer that orders or filters journal entries by `tb_responses.dttm_created_utc` will read corrected values; consumers that already ran on the skewed data have no persisted derived state in this codebase.
