...
async function main() {
  ...
  await updateProfile(subjectId, latestQuestionId);

  // Verify
  const profile = await sql`SELECT session_count, vocab_cumulative, burst_count_mean, ex_gaussian_tau_mean FROM tb_personal_profile WHERE subject_id = ${subjectId} LIMIT 1`;
  if (profile.length > 0) {
