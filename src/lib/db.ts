...
export { default as sql } from './db-pool.ts';
...
export async function getCalibrationBaselines(deviceType?: string | null, hourOfDay?: number | null): Promise<CalibrationBaseline> {
  // Build dynamic filter fragments
  const deviceFilter = deviceType ? sql`AND s.device_type = ${deviceType}` : sql``;
  const hourFilter = hourOfDay != null
    ? sql`AND (ABS(s.hour_of_day - ${hourOfDay}) <= 2 OR ABS(s.hour_of_day - ${hourOfDay}) >= 22)`
    : sql``;
  ...
  const contextRows = await sql`
    ...
    ${hourFilter}
  `;
  let row = contextRows[0] as Omit<CalibrationBaseline, 'confidence'>;

  // Fall back to global if context-matched has too few
  if (row.sessionCount < 3 && (deviceType || hourOfDay != null)) {
    const globalRows = await sql`
      ...
      WHERE q.question_source_id = 3
    `;
    row = globalRows[0] as typeof row;
  }
