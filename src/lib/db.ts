...
// - zz_archive_ = preserved historical tables, no longer written or read
...
// ----------------------------------------------------------------------------
// ARCHIVE INDEX (data preserved, no longer in active pipeline)
// ----------------------------------------------------------------------------
// 2026-04-16 interpretive-layer restructure (slices 1+2):
...
//   Reason: predict-and-grade on N=1 self-built data has three-loop circularity
//   (designer = subject = stimulus source) that deterministic grounding can't fix.
//   Migration: scripts/archive-interpretive-layer-20260416.ts
//
// 2026-04-16 slice 3 (PersDyn 8D → 7D + parallel semantic space):
//   tb_entry_states (8D)          → zz_archive_entry_states_8d_20260416
//   tb_trait_dynamics (8D)        → zz_archive_trait_dynamics_8d_20260416
//   tb_coupling_matrix (8D)       → zz_archive_coupling_matrix_8d_20260416
//   Reason: `expression` was relocated from PersDyn into a parallel first-class
//   semantic state space (tb_semantic_states). Behavioral 7D and semantic ND
//   are kept orthogonal at construction time. Coupling and dynamics are now
//   computed separately on each space.
//   Migration: scripts/archive-8d-state-20260416.ts
//
// All archived data is intact. Stub functions for archived tables return
// empty results / no-ops to keep call sites compiling while the layer is
// piecewise removed. Search for "// archived" to find them.
// ----------------------------------------------------------------------------
...
db.exec(`
  ...
  -- tb_reflections
  -- --------------------------------------------------------------------------
  -- PURPOSE: Per-week structured signal digest (compact behavioral signals +
  --          dynamics summary + calibration confidence + delta trend). NOT
  --          narrative anymore.
  -- HISTORY: Pre-2026-04-16 this stored 2-3K-token narrative reflections
  --          generated by Opus with Sonnet audit. That layer was removed in
  --          the slice-1 interpretive-layer restructure. The text column now
  --          holds a deterministic structured-receipt format produced by
  --          src/lib/reflect.ts with no LLM call.
  -- USE CASE: Embedded for semantic retrieval at question-generation time so
  --           past digests can resurface when their themes are relevant.
...
db.exec(`
  ...
  -- tb_witness_states
  -- --------------------------------------------------------------------------
  -- PURPOSE: Store the witness-form's AI-generated trait vectors used to
  --          render the Alice Negative 3D figure.
  -- STATUS: As of slice 3 (2026-04-16) this rendering path is being
  --         repurposed from a user-facing aesthetic surface into a
  --         designer-only coupling-graph / mode-landscape visualization.
  --         The table and writer (src/lib/alice-negative/render-witness.ts)
  --         still run on each session submit; the consumer surface is
  --         being redesigned. Do not invest further in user-visible
  --         witness-rendering UI.
  -- USE CASE: "What trait vector did the renderer produce after entry N?"
