...
const existingBehavioral = getEntryStateCount();
...
const existingSemantic = getSemanticStateCount();
...
if (behavioralStates.length >= 5) {
  ...
  saveTraitDynamics(behDynamics.dimensions.map(d => ({
  ...
  if (behDynamics.coupling.length > 0) {
    saveCouplingMatrix(behDynamics.coupling.map(c => ({
      entry_count: behavioralStates.length,
      leader: c.leader,
      follower: c.follower,
...
if (semanticStates.length >= 5) {
  ...
  saveSemanticDynamics(semDynamics.dimensions.map(d => ({
  ...
  if (semDynamics.coupling.length > 0) {
    saveSemanticCoupling(semDynamics.coupling.map(c => ({
      entry_count: semanticStates.length,
      leader: c.leader,
      follower: c.follower,
      lag_sessions: c.lagSessions,
...
`).all() as Array<{ question_id: number; hourOfDay: number | null; totalDurationMs: number | null; deletionEventsJson: string | null }>;

const alreadyComputedMeta = getMetadataQuestionIdsAlreadyComputed();
let metadataAdded = 0;
...
for (const sess of allSessions) {
  ...
  const bursts = getBurstSequence(sess.question_id);
...
`).all() as Array<{ question_id: number; device_type: string | null }>;

const existingCalSnapshots = (db.prepare(
  'SELECT COUNT(*) as c FROM tb_calibration_baselines_history'
).get() as { c: number }).c;
