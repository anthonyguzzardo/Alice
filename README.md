...
On days with both a calibration and journal session, the system computes a **same-day session delta** — the behavioral difference between neutral and reflective writing, controlling for every daily confound (sleep, stress, device, time-of-day). This implements Pennebaker's (1986) expressive writing paradigm: the only variable that changed was the prompt. The delta tracks 8 research-backed dimensions: first-person density (Newman 2003), cognitive density (Vrij), hedging density, typing speed, commitment ratio, large deletions (Faigley & Witte), keystroke interval (Epp et al.), and P-burst length (Chenoweth & Hayes). After 15+ days of history, the system contextualizes whether today's delta is within or outside the person's typical range — a widening delta means the question provoked more behavioral shift than usual.
...
- **Signal registry** (`src/lib/signal-registry.ts`) — canonical vocabulary of ~90 deterministic signals across three sources: session (keystroke dynamics, linguistic densities, production metrics), delta (same-day calibration-to-journal behavioral shift), and dynamics (8D PersDyn model parameters). Every signal is code-computed — no LLM involvement. The registry is injected into the prediction prompt so the LLM knows which signal names are valid when constructing structured criteria.
