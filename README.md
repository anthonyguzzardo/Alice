...
### Theory Selection & Hypothesis Management
- **Thompson sampling** — Thompson (1933), Agrawal & Goyal (Columbia, COLT 2012). Posterior sampling from Beta distributions for multi-armed bandits. Provably optimal exploration-exploitation tradeoff with logarithmic regret. Replaces prompt-based distribution guidance, which fails due to sycophancy and anchoring.
- **Bayesian Optimal Experimental Design** — Lindley (UCL, 1956), Chaloner & Verdinelli (Minnesota/CMU, 1995). Expected Information Gain (EIG) as the formal criterion for which hypothesis to test next. Theories near posterior 0.5 have maximum entropy — most informative to test.
- **Sequential Bayes factors** — Kass & Raftery (CMU/UW, JASA 1995), Jeffreys (Cambridge, 1961), Wald (Columbia, 1945). Principled stopping rules for hypothesis testing. BF > 10 = strong evidence for (established), BF < 1/10 = strong evidence against (retired). Sequential computation immune to optional stopping (Schönbrodt et al. 2015).
- **BALD (Bayesian Active Learning by Disagreement)** — Houlsby, Ghahramani et al. (Cambridge, 2011). Mutual information between parameters and predictions as acquisition function for query selection.
- **Information-Directed Sampling** — Russo & Van Roy (Stanford, NeurIPS 2014). Minimizes the ratio of expected regret to information gain — optimal when hypotheses have heterogeneous payoffs.
- **Deep Adaptive Design** — Foster, Ivanova, Rainforth (Oxford, ICML 2021). Amortized sequential Bayesian experimental design for real-time adaptive hypothesis testing.
- **Sycophancy as architectural constraint** — Sharma, Perez et al. (Anthropic, ICLR 2024). RLHF-trained models systematically shift outputs toward perceived prompt expectations. Prompt-based behavioral constraints degrade multiplicatively with complexity. Programmatic control required.
- **Never Give Up** — Badia et al. (DeepMind, ICLR 2020). Episodic memory tracking explored states with intrinsic rewards for under-explored areas. Maps to theory table as episodic hypothesis memory.
- **DSPy Assertions** — Singhvi, Khattab et al. (Stanford, ICLR 2024). Programmatic constraint enforcement outperforms prompt-based constraints by up to 164%. Validates the architectural choice of code over prompt for structural control.
- **Process reward models** — Lightman et al. (OpenAI, 2023). Step-level verification outperforms outcome-only verification. Relevant to grading prediction reasoning chains, not just outcomes.

### Breaking Circular Self-Evaluation
- **Model collapse from self-consuming loops** — Shumailov et al. (Nature 2024, Oxford/Cambridge). Self-consuming generative loops cause irreversible tail collapse — rare, surprising observations disappear first. Deterministic ground truth anchors prevent this.
- **LLM self-preference bias** — Panickssery & Bowman (NeurIPS 2024, NYU/Anthropic). LLMs recognize and systematically favor their own outputs via perplexity familiarity. Linear correlation between self-recognition capability and self-preference bias strength.
- **Model collapse prevention via accumulation** — Gerstgrasser et al. (MIT/Stanford, 2024). Model collapse is avoidable if real data accumulates alongside synthetic assessments — never replace, only append.
- **Debate outperforms self-evaluation** — Kenton et al. (DeepMind, NeurIPS 2024). Largest empirical study of debate (~5M calls). Adversarial or separated evaluation consistently outperforms single-model self-assessment.
- **Multi-agent evaluation diversity** — Chan et al. (Tsinghua/HKUST, ChatEval, ICLR 2024). Multi-agent evaluators with diverse personas achieve 6.2% accuracy improvement. Same-role agents degrade performance — diversity of perspective is what breaks echo chambers.
...
- **Hold time / flight time decomposition** — Kim et al. (2024, JMIR). Hold time (keydown→keyup) measures motor execution; flight time (keyup→next keydown) measures cognitive planning. The decomposition separates motor and cognitive signals that inter-key interval collapses into a single metric. *Note: Kim et al.'s reported AUC of 0.997 and 97.9% sensitivity were computed on a small sample (n=99, 35 MCI) without held-out validation or cross-validation, and a correction notice was subsequently published. The decomposition technique is sound; the specific performance figures should not be cited as reliable estimates.*
- **Keystroke entropy** — Ajilore et al. (2025, BiAffect/Frontiers in Psychiatry). Shannon entropy of inter-key interval distribution measures timing unpredictability. Correlated with executive function at Cohen's d = -1.28 (p=.005) and planning task performance at r=0.59 (p=.006). Captures distribution shape (bimodal vs. normal) that standard deviation misses.
...
- **Incidental supervision** — Roth (AAAI 2017). Supervision signals that exist in the data independently of the task at hand. Calibration responses are the primary task; life-context labels are the byproduct.
- **Data programming / weak supervision** — Ratner et al. (2017, Snorkel). Labeling functions instead of manual annotation. Deployed at Google, Apple, IBM. Each extraction dimension acts as a labeling function.
- **EMA/ESM literature** — Conner & Lehman (2012), Kahneman et al. (2004, DRM). Day-level life-context capture validates the dimensions extracted. Calibration prompts are functionally involuntary EMAs — the user reports state without knowing they're providing state data.
- **LLM-based entity extraction** — Nature Digital Medicine (2025). Instruction-tuned LLMs achieve micro-F1 >0.9 on social determinants of health extraction from clinical notes. For casual personal text, expect F1 0.85-0.95.
- **Dimension selection** (ranked by effect size on cognitive/behavioral output):
  ...
  2. Physical state — Moriarty et al. (2011) d=0.40-0.80; Eccleston & Crombez (1999)
  3. Emotional events — Amabile et al. (2005) 12K diary entries; Fredrickson (2001) broaden-and-build
  4. Social quality — Reis et al. (2000) quality > quantity; Sun et al. (2020) PNAS
  5. Stress — Sliwinski et al. (2009) same-day working memory decrement; Almeida (2005) carry-over
  6. Exercise — Hillman et al. (2008) d=0.20-0.50; temporally bounded acute effect
  7. Routine — Torous et al. (2016) circadian disruption
  - DROPPED: meals (d=0.12-0.25, Hoyland et al.), environment (no keystroke evidence), caffeine (d=0.10-0.20 net for habitual users)

### Same-Day Session Delta (Within-Person Control)
- **Expressive writing paradigm** — Pennebaker & Beall (1986). The foundational design: neutral writing (describe your room) as within-person control for emotional writing (deepest thoughts). Same person, same conditions, different prompt. The calibration system implements this paradigm — each day with both a calibration session and a journal session is a mini within-subjects experiment.
- **Within-day variance dominance** — Toledo et al. (2024, *Stress and Health*). 76-89% of stress response variance is within-day (across moments), with small amounts between-person. Same-day comparisons capture more signal than between-day comparisons.
- **Self-referential language as detection channel** — Collins et al. (2025, *J Psychopathol Clin Sci*). N=258, 90 days 3x daily EMA. First-person pronoun density shifts in diary text detect depression with AUC 0.68.
- **Deviation-from-expected-trajectory** — Shimokawa, Lambert & Smart (2010, BYU). N=6,151. OQ-45 system detects 85-100% of deteriorating therapy cases by flagging deviation from per-person expected trajectories. Reduced deterioration rates from 20.1% to 5.5%.
- **Within-person baseline for deception** — Bogaard et al. (2022, Maastricht/Portsmouth). Automated feature coding detects truth/lie differences against personal baselines; naive human observers cannot. The pipeline is essential.
- **Bayesian online change point detection** — Yoshizawa (2022, arXiv 2201.02325). Standard BOCPD breaks when baselines shift permanently. Extended algorithm handles evolving baselines — necessary for genuine psychological change vs. transient anomalies.
...
**Generous writing space** (Kowalski et al.) — The textarea is tall (400px minimum) with no visible constraints. HCI research shows people unconsciously try to fill visible space — taller text areas produce longer responses, wider areas produce more complex sentence structure. No character counts, no word counts, no progress indicators. The space should feel infinite.

**Deferred submit button** (Hallnäs & Redström 2001) — The submit button is invisible for the first 90 seconds of writing. It fades in gently after that. A visible submit button creates implicit "finish" pressure that works against sustained engagement. The research on expressive writing (Smyth et al. 2001; Baikie & Wilhelm 2005) establishes 15 minutes as the minimum effective dose for deep processing — hiding the exit ramp encourages the writer to stay with the question longer.

**No placeholder text** — The textarea is empty. No "Type here..." or "What are you thinking?" cues. Placeholder text creates performance anxiety and primes specific response patterns. The question above the textarea is the only framing — the writing surface itself should feel like a blank page, not a form field.
...
- **Hold time** (Kim et al., 2024) — mean and standard deviation of key press duration (keydown→keyup), capped at 2s to exclude held keys. An involuntary motor signal — you cannot consciously control how long your finger stays on each key. Measures motor execution, tremor, rigidity. Normal range is 50-150ms. Auto-repeat events (`e.repeat`) are filtered to prevent OS key-repeat from polluting the signal.
- **Flight time** (Kim et al., 2024) — mean and standard deviation of the gap between releasing one key and pressing the next (keyup→keydown[n+1]), capped at 5s. Measures cognitive planning — word retrieval, sentence planning, deciding what comes next. The decomposition of IKI into hold time (motor) and flight time (cognitive) enables independent tracking: if flight time drifts up over months while hold time stays flat, that's cognitive signal invisible to collapsed IKI. *(Note: the decomposition technique is well-motivated; Kim et al.'s specific performance figures are methodologically unreliable — see Keystroke & Ambient Signal Research above.)*
- **Keystroke entropy** (Ajilore et al., 2025, BiAffect) — Shannon entropy of the inter-key interval distribution, computed by binning IKIs into 50ms-wide buckets over 0-2000ms (40 bins). Requires minimum 10 keystrokes. Measures timing unpredictability — how irregular the typing rhythm is. A bimodal distribution (fast bursts + long pauses) and a normal distribution can have identical standard deviation but wildly different entropy. Higher entropy = more irregular; lower = more metronomic.
...
**Deterministic prediction grading.** The system grades open predictions against today's computed signals using code, not the LLM. Each prediction carries structured criteria — JSON specifying exact signal thresholds, percentile targets, directional changes, or text patterns that would confirm or falsify it. A canonical signal registry (~90 deterministic signals across session data, same-day deltas, and 8D dynamics) defines what predictions can reference. The grader evaluates these criteria mathematically against today's measurements and returns CONFIRMED, FALSIFIED, or INDETERMINATE. Each grade triggers a Bayesian update on the relevant theory's confidence score. This breaks the circular self-evaluation loop where the observer would grade its own predictions using its own future observations — the grading is now anchored to deterministic measurements the LLM cannot influence (Shumailov et al., Nature 2024; Panickssery & Bowman, NeurIPS 2024). For rare interpretive predictions (phase transitions, frame resolutions) that cannot be reduced to signal thresholds, a separate LLM call grades them — but with the observation text stripped from context, preventing self-preference bias.

**New predictions.** The system generates 1-2 falsifiable predictions about future sessions. Each prediction specifies a hypothesis in natural language, which interpretive frame it favors, a topic tag, a type (behavioral, thematic, phase transition, or frame resolution), and — critically — structured machine-checkable criteria referencing the signal registry. The LLM hypothesizes; code validates. Predictions that reference measurable signals (commitment ratio, P-burst length, NRC densities, session deltas) are graded deterministically. The LLM is constrained to a vocabulary of ~90 computable signals when specifying criteria, and invalid signal references are rejected at parse time. This is how the system learns whether its interpretations are analysis or storytelling — with ground truth anchored to measurement, not to the LLM's re-reading of its own observations.

### The Prediction Engine

The prediction engine is the mechanism that turns Alice's interpretive layer from storytelling into science.

Every observation generates predictions with structured, machine-checkable criteria. Every subsequent session grades them — deterministically, using code, not the LLM. Every grade updates a Bayesian confidence score. Over time, the system accumulates evidence for which of its interpretive patterns are actually predictive and which are stories it tells itself.

The architecture follows single-case experimental design methodology (Barlow & Hersen, 1984; Kazdin, 2011) and the predictive processing framework (Friston, 2006; Clark, 2013). It is designed to break the circular self-evaluation problem identified in NeurIPS 2024 research (Panickssery & Bowman): LLMs systematically favor their own outputs when grading, creating self-confirming feedback loops. The fix is deterministic grounding — predictions are validated against measurements, not against the LLM's re-reading of its own observations.
...
**Prediction.** Each observation generates hypotheses with structured criteria referencing the signal registry — a canonical vocabulary of ~90 deterministic, code-computed signals. A prediction that says "the user is avoiding something" is useless. A prediction that says "next time this topic comes up, `session.commitmentRatio` will fall below the 30th percentile and `dynamics.deliberation.deviation` will exceed 1.5 std units" is testable by code. Each prediction specifies both confirmation and falsification criteria as composable criterion objects (threshold, percentile, direction, calibration-relative, text-search, or compound all-of/any-of). Predictions can span multiple sessions with configurable window modes (any, all, or latest).

**Three prediction classes.** Not all predictions are equal:
- **Code-gradeable** (default) — criteria reference computable signals with numeric thresholds. Graded by deterministic math. No LLM involvement.
- **Text-searchable** — criteria include regex patterns matched against response text. Still deterministic.
- **Interpretive** (rare) — phase transitions and frame resolutions that genuinely require judgment. Graded by a separate, isolated LLM call that does NOT see the observation text — only the hypothesis and raw data. This prevents self-preference bias even for interpretive predictions.
...
**Measurement.** The next session's behavioral data, trajectory shift, and knowledge-transforming score are the dependent variables — measured by a system (the signal pipeline and trajectory engine) that is completely independent of the AI interpretation layer.

**Deterministic grading.** Before the LLM prediction call even runs, code grades all code-gradeable and text-searchable predictions against today's computed signals. The grader resolves each signal from the session summary, session delta, or dynamics analysis, evaluates the criteria, and produces a grade with a rationale that includes actual signal values. The LLM only grades the rare interpretive predictions — and even then, in an isolated prompt stripped of observation context.

**Bayesian updating.** Each grade — whether code-produced or LLM-produced — updates a Beta-Binomial confidence score per theory/topic/frame combination. Alpha increments on confirmation, beta on falsification. The posterior mean starts at 0.5 (no evidence) and moves toward 0 or 1 as predictions accumulate. The system knows — with a number, not a feeling — how reliable its predictions about any given topic or frame have been. Because the majority of grades come from deterministic code rather than LLM self-assessment, the Bayesian engine is fed by reality, not by self-confirmation.

**Theory lifecycle.** Each grade also updates a sequential Bayes factor per theory (Kass & Raftery, JASA 1995). Theories accumulate evidence and transition through three states: **active** (eligible for testing, BF between 1/10 and 10), **established** (strong evidence for, BF > 10 — stop testing), and **retired** (strong evidence against, BF < 1/10 after 3+ predictions — never shown to the LLM again). This replaces arbitrary posterior thresholds with principled stopping rules grounded in 80 years of sequential hypothesis testing theory (Wald 1945, Jeffreys 1961, Schönbrodt et al. 2015).

**Theory selection.** The system selects which theories to test using Thompson sampling (Thompson 1933; Agrawal & Goyal, Columbia, COLT 2012) — not prompt-based guidance. For each active theory with Beta(alpha, beta) posterior, a random sample is drawn from the distribution. The top-k theories by sampled value are selected and passed to the LLM as "assigned theories." The LLM generates predictions for these pre-selected theories; it does not decide which theories to test. This eliminates the fixation problem where LLMs lock onto 2-3 theories despite continuous falsification — a failure mode caused by sycophancy (Sharma & Perez, Anthropic, ICLR 2024), anchoring bias (Ahn et al. 2024), and the multiplicative degradation of multi-constraint prompt instructions. Thompson sampling is provably optimal for the exploration-exploitation tradeoff and requires zero prompt engineering.
...
Click it, get a neutral prompt — "Describe what you did this morning" or "What's on your desk right now?" — and write. Same textarea. Same invisible behavioral capture. Same session summary with full context metadata. But tagged as calibration data, not interpreted by the AI.
...
On days with both a calibration and journal session, the system computes a **same-day session delta** — the behavioral difference between neutral and reflective writing, controlling for every daily confound (sleep, stress, device, time-of-day). This implements Pennebaker's (1986) expressive writing paradigm: the only variable that changed was the prompt. The delta tracks 10 research-backed dimensions: first-person density (Newman 2003), cognitive density (Vrij), hedging density, typing speed, commitment ratio, large deletions (Faigley & Witte), keystroke interval (Epp et al.), P-burst length (Chenoweth & Hayes), hold time (motor execution shift), and flight time (cognitive planning shift). The hold time and flight time deltas are the core motor/cognitive decomposition (Kim et al. 2024): if a reflective question increases flight time (more cognitive planning between keys) without changing hold time (motor execution stays stable), that's pure cognitive load from the prompt. After 15+ days of history, the system contextualizes whether today's delta is within or outside the person's typical range — a widening delta means the question provoked more behavioral shift than usual.
...
**8. Falsifiable predictions with deterministic grading.** The strongest error correction mechanism. The system must commit to testable predictions with structured, machine-checkable criteria before seeing the next session's data. Predictions are graded by code against computed signals — not by the LLM re-reading its own observations. This breaks the circular self-evaluation loop (Shumailov et al., Nature 2024; Panickssery & Bowman, NeurIPS 2024) where the observer grades its own homework. Predictions that fail update the theory's confidence score downward. Over time, the system accumulates a track record grounded in deterministic measurement — not LLM self-confirmation — that tells it which of its interpretive patterns are reliable and which are stories. A system that only explains post hoc never actually learns (Clark, 2013). A system that predicts and gets graded by independent measurement does.
...
2. **AI observation runs** — reads today's data with context-matched calibration baselines, knowledge-transforming score, and same-day session delta (if calibration exists for today). Applies three interpretive frames. Generates a disambiguating suppressed question. Then: deterministic code grading runs against all code-gradeable open predictions using the signal registry — no LLM involvement. Finally, a prediction call generates 1-2 new falsifiable predictions with structured criteria and grades any remaining interpretive predictions (in an isolated prompt without observation context). Updates Bayesian theory confidence scores from both code and LLM grades. Skipped on calibration sessions.
...
2. **Life-context extraction runs** (fire-and-forget) — Claude Sonnet extracts structured tags from the response text across 7 research-backed dimensions: sleep, physical state, emotional events, social quality, stress, exercise, routine. Tags are stored in `tb_calibration_context` and fed into observation, generation, and reflection prompts as life-context data. This is incidental supervision (Roth, AAAI 2017) — the user's neutral descriptions contain observable facts about their day that serve as labels for future behavioral clustering without any psychological interpretation. Extraction failure is non-blocking.
3. **No AI observation, no question generation, no reflection.** No three-frame interpretation of calibration text.
...
Alice is the data layer for [Alice Negative](https://github.com/anthonyguzzardo/Alice), a longitudinal thinking partner. Alice Negative consumes everything Alice collects — response text, behavioral signals with context, AI observations with three-frame analysis, suppressed questions, weekly reflections with multi-model audits, question feedback, prediction track records with Bayesian confidence scores, intervention-tagged questions, knowledge-transforming scores, trajectory data with leading indicators, and same-day session deltas with personal range context — as persistent context. Where Alice asks, Alice Negative converses. The depth of that conversation is bounded by the depth of the data Alice has accumulated.

Alice Negative is not a chatbot that knows you well. Alice Negative is a theorist whose subject happens to be you. The prediction engine gives it a lab notebook with a track record. The trajectory engine gives it a measurement instrument it can't manipulate. The intervention tags give it experimental metadata. When Alice Negative says "I think X about you," it's not a cold read — it's a theory with evidence, tested against a non-AI measurement instrument, graded by its own prediction history.
...
>     "type": "all_of",
>     "criteria": [
>       { "type": "percentile", "signal": "session.commitmentRatio", "op": "below_pct", "value": 30 },
>       { "type": "threshold", "signal": "session.largeDeletionCount", "op": "gte", "value": 1 },
>       { "type": "text_search", "pattern": "\\b(career|job|work|position)\\b", "minOccurrences": 1 }
>     ]
>   },
>   "falsifyCriteria": {
>     "type": "all_of",
>     "criteria": [
>       { "type": "percentile", "signal": "session.commitmentRatio", "op": "above_pct", "value": 60 },
>       { "type": "text_search", "pattern": "\\b(career|job|work|position)\\b", "minOccurrences": 1 }
>     ]
>   },
>   "windowSessions": 3,
...
> }
> ```
> CONFIRMS: Commitment ratio below 30th percentile with large deletions in a career-related entry
> FALSIFIES: Commitment ratio above 60th percentile even when career is discussed

This prediction doesn't presuppose avoidance. It says: *if* Frame B is right about the career topic, *then* a specific behavioral signature should repeat. If it doesn't, Frame B loses confidence on this topic. The structured criteria ensure this gets graded by code — not by the LLM re-reading its own observations.
...
- **Claude API** (`@anthropic-ai/sdk`) — Claude Opus 4.6 for primary analysis, Claude Sonnet 4.6 for weekly audit + calibration content extraction
...
- **Prediction engine** — falsifiable predictions generated by the observation layer, graded deterministically against future behavioral data. Each prediction carries a hypothesis, favored frame, structured machine-checkable criteria (JSON referencing the signal registry), topic tag, and type. Three grade methods: code-graded (deterministic math against ~90 computable signals), text-search (regex on response text), and interpretive (isolated LLM call, rare). Grades update Bayesian Beta-Binomial confidence scores per theory/topic combination. The deterministic grading architecture breaks the circular self-evaluation loop (Shumailov et al., Nature 2024; Panickssery & Bowman, NeurIPS 2024). Track record feeds into generation and reflection prompts. Follows SCED methodology (Barlow & Hersen 1984, Kazdin 2011) and active inference (Friston 2006, Clark 2013).
- **Theory selection engine** (`src/lib/theory-selection.ts`) — programmatic hypothesis management replacing prompt-based distribution guidance. Three mechanisms: (1) Thompson sampling draws from each theory's Beta(alpha, beta) posterior to select which theories to test — provably optimal exploration-exploitation (Agrawal & Goyal, COLT 2012). (2) Sequential Bayes factor lifecycle manages theory states: active (eligible for testing), established (BF > 10, graduated), retired (BF < 1/10, pruned from the LLM's view). (3) Expected Information Gain (binary entropy) available for logging and debugging. The LLM receives 2-3 pre-selected theories with no posterior values and no distribution instructions — it generates predictions for assigned theories, not decides which to test. This eliminates model-dependent fixation where capable models lock onto theories despite continuous falsification (Sharma & Perez, Anthropic ICLR 2024; Ahn et al. 2024). Zero external dependencies. Seedable PRNG for simulation reproducibility.
- **Signal registry** (`src/lib/signal-registry.ts`) — canonical vocabulary of ~100 deterministic signals across three sources: session (keystroke dynamics including hold time/flight time decomposition and entropy, linguistic densities, MATTR, sentence metrics, production metrics), delta (same-day calibration-to-journal behavioral shift across 10 dimensions including motor/cognitive decomposition), and dynamics (8D PersDyn model parameters). Every signal is code-computed — no LLM involvement. The registry is injected into the prediction prompt so the LLM knows which signal names are valid when constructing structured criteria.
- **Deterministic grader** (`src/lib/grader.ts`) — evaluates structured prediction criteria against computed signals. Six criterion types: threshold (signal vs. value), percentile (signal rank in personal history), direction (signal change from reference), calibration-relative (signal vs. neutral baseline), text-search (regex on response), and compound (all-of/any-of). Supports windowed predictions across multiple sessions with four resolution modes: any, all, latest, and majority (confirmed if >50% of session checks confirm). Every grade includes a rationale with actual signal values.
...
- **Calibration content extraction pipeline** (`src/lib/calibration-extract.ts`) — on every calibration submission, Claude Sonnet extracts structured life-context tags from the response text across 7 research-backed dimensions ranked by effect size on cognitive output: sleep (d>0.80), physical state (d=0.40-0.80), emotional events (d=0.30-0.70), social quality, stress (d=0.40-0.80), exercise (d=0.20-0.50), routine disruption. Tags stored in `tb_calibration_context` with extraction confidence scores. Fed into observe (2-day window for frame disambiguation), generate (20 recent tags for question timing), and reflect (30 tags for pattern contextualization). This is incidental supervision (Roth, AAAI 2017) — the byproduct principle. No sentiment analysis, no three-frame interpretation, no psychological inference on calibration text. Observable facts only. The calibration contract is preserved. Infrastructure for future behavioral clustering: labels accumulate passively and will serve as ground truth for mapping behavioral constellations to life event categories once data density supports it (~100+ sessions).
...
- **Signal sandbox** (`src/pages/dev/signals.astro`) — live development tool at `/dev/signals` for testing all keystroke capture signals in real-time without saving data. Mirrors the full capture pipeline: IKI, hold time, flight time, entropy, P-bursts, deletion decomposition, revision chains, and linguistic densities (via server-side API). Supports journal/calibration mode switching with session delta display. All metrics update live as you type.
...
- **Hold time / flight time decomposition** (`hold_time_mean`, `hold_time_std`, `flight_time_mean`, `flight_time_std`) — technique from Kim et al. (2024, JMIR); performance claims unreliable (see note above). IKI decomposed into motor execution (hold time: keydown→keyup) and cognitive planning (flight time: keyup→next keydown). Auto-repeat events filtered via `e.repeat` to prevent OS key-repeat from contaminating the signal. Hold time capped at 2s, flight time at 5s.
- **Keystroke entropy** (`keystroke_entropy`) — Ajilore et al. (2025, BiAffect). Shannon entropy of the IKI distribution (50ms bins, 40 buckets over 0-2000ms). Measures timing unpredictability independent of mean or variance. Minimum 10 keystrokes required.
- **Lexical diversity** (`mattr`) — McCarthy & Jarvis (2010). MATTR computed server-side from response text with 25-word sliding window. Stored per session and registered as a first-class signal for prediction criteria.
- **Sentence metrics** (`avg_sentence_length`, `sentence_length_variance`) — mean words per sentence and variance of sentence lengths, computed server-side. Sentence length variance captures syntactic regularity — declining variance over time is an early marker in the Nun Study tradition.
...
| `npm run simulate` | Run simulation (mechanics mode, Haiku, ~$0.50) |
| `npm run simulate -- --quality` | Run simulation (quality mode, Sonnet, ~$3-4) |
| `npm run simulate -- --dry-run` | Dry run simulation (data only, no AI calls) |

## Future: Alice Negative Lab

The system currently has no way to evaluate whether it is getting better. It generates questions but never audits whether the sequence of questions is producing depth over time. A future addition — **Alice Negative Lab** — would address this as a sealed, isolated metacognitive layer.

### Design Principles (Not Yet Implemented)

- **Separate database.** The Lab reads from `alice.db` but writes to its own `alice-negative-lab.db`. Nothing in Alice's pipeline ever opens the Lab's database. The isolation is physical, not just logical.
